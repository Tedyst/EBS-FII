...
class TestComparableFilter(unittest.TestCase):
    def setUp(self):
        ...
        self.subscription3 = Subscription(
            id="sub3",
            stationid=Comparable(
                value=1, comparator=Comparator.GREATER
        ...
        self.filter.add_subscription(
            self.subscription3
        )  # Should be ignored due to different comparator
...
class TestAllComparableFilter(unittest.TestCase):
    def setUp(self):
        ...
        self.sub_equal = Subscription(
            id="equal", temp=Comparable(value=20, comparator=Comparator.EQUAL)
        )
        self.sub_greater = Subscription(
            id="greater", temp=Comparable(value=15, comparator=Comparator.GREATER)
        )
        self.sub_less = Subscription(
            id="less", temp=Comparable(value=25, comparator=Comparator.LESS)
        )
        self.sub_no_temp = Subscription(
            id="no_temp", stationid=Comparable(value=1, comparator=Comparator.EQUAL)
        )
    ...
    def test_add_and_match_subscription(self):
        ...
        self.assertIn(
            self.sub_less, matches
        )  # For LESS comparator, it checks if pub.temp < sub.value, which is 20 < 25, so it should match
